...
void circuit() {
    ...
    // Applico Hadamard a tutti i qubit per creare una sovrapposizione
    for (int i = 0; i < numQubits; i++) {
        applyHadamard(state, i);
    }

    // Stampa lo stato prima della misura
    printf("Stato del sistema prima della misura:\n");
    printState(state);

    // Esegui la misura sul qubit 0
    int measureResult = measure(state, 0);
    printf("\nRisultato della misura sul qubit 0: %d\n", measureResult);

    // Stampa lo stato dopo la misura
    printf("\nStato del sistema dopo la misura:\n");
    printState(state);
